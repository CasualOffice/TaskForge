...
| **ADR-024** | Bundle budget 200 KB, measured before it is promised | Accepted | The authenticated shell budget is enforced in CI. The *floor* is measured in Phase 0 against the real dependency set; if the floor is genuinely higher, the number is raised by superseding ADR — never quietly exceeded. See [42](42-FRONTEND-ARCHITECTURE.md). |
