...
#
# Usage: ./run-gates.sh [cargo test args...]
#   no args -> the full gate set (check.sh + every ignored test)
#   args    -> just `cargo test <args> -- --ignored --test-threads=1`
set -uo pipefail
cd "$(dirname "$0")"
...
if [ "$#" -gt 0 ]; then
  cargo test "$@" -- --ignored --test-threads=1 2>&1 | tail -80
  status=${PIPESTATUS[0]}
